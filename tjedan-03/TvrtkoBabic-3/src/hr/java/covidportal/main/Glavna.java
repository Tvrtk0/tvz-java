...
public class Glavna {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < BR_BOLESTI; i++) {
            ...
            try {
                provjeraBolestIstihSimptoma(bolesti, i);
            }
     ...
     * @param kontaktiraneOsobe polje osoba s kojim je trenutna osoba bila u kontaktu (ako postoje)
                                                   ...
                                                   Osoba[] kontaktiraneOsobe, Integer brKontaktiranihOsoba)
    {
        ...
        for (int j = 0; j < brKontaktiranihOsoba; j++) {
            ...
            kontaktiraneOsobe[j] = osobe[indexKontakta];

            try {
                logger.info("Odabran je kontakt: " + osobe[indexKontakta].getPrezime());
                provjeraKontaktiranihOsoba(kontaktiraneOsobe, j + 1);
            }
    ...
    public static void provjeraKontaktiranihOsoba (Osoba[] kontaktiraneOsobe, int brKontaktiranihOsoba)
            throws DuplikatKontaktiraneOsobe {

        for (int i = 0; i < brKontaktiranihOsoba - 1; i++) {
            for (int j = i + 1; j < brKontaktiranihOsoba; j++) {
                if (kontaktiraneOsobe[i].equals(kontaktiraneOsobe[j]))
     ...
     * @param index index na kojem se nalazi trenutna bolest ili virus za koju je potrebna provjera.
     * @throws BolestIstihSimptoma iznimka koja se basa u slučaju kad je unesena bolest ili virus sa istim simptomima
     * koje je imala prije unešena bolest ili virus
     */

    public static void provjeraBolestIstihSimptoma(Bolest[] bolesti, int index)
            throws BolestIstihSimptoma {
        Simptom[] simptomi = bolesti[index].getSimptomi();

        for (int i = 0; i < index; i++) {
            Simptom[] tempSimptomi = bolesti[i].getSimptomi();
            int br = 0;
            if (simptomi.length == tempSimptomi.length) {

                for (int j = 0; j < simptomi.length; j++) {
                    for (int k = 0; k < simptomi.length; k++) {
                        if (tempSimptomi[j].getNaziv().equals(simptomi[k].getNaziv()))
                            br++;
                    }
                }
                if (br == simptomi.length)
